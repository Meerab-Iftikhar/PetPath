...
const Pets = () => {
  ...
  useEffect(() => {
    const fetchApprovedPets = async () => {
      try {
        const response = await axios.get('${import.meta.env.VITE_API_URL}/api/adoption/get-approved-pets');
  ...
  const handleAdoptionRequest = async (event) => {
    ...
    try {
      const response = await axios.post('${import.meta.env.VITE_API_URL}/api/adopting/submit-adoption-request', data);
