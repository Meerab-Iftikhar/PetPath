...
const AdoptionReq = () => {
  ...
  useEffect(() => {
    const fetchAdoptionRequests = async () => {
      try {
        const response = await axios.get('http://43.204.231.18:6000/api/adopting/adoption-requests');
  ...
  const handleReject = async (id) => {
    try {
      await axios.delete(`http://43.204.231.18:6000/api/adopting/adoption-requests/${id}`);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await axios.put(`http://43.204.231.18:6000/api/adopting/adoption-requests/${id}`, {
        status: 'Adopted',
      });
