...
const AdoptionReq = () => {
  ...
  useEffect(() => {
    const fetchAdoptionRequests = async () => {
      try {
        const response = await axios.get('http://13.201.98.216:4000/api/adopting/adoption-requests');
  ...
  const handleReject = async (id) => {
    try {
      await axios.delete(`http://13.201.98.216:4000/api/adopting/adoption-requests/${id}`);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await axios.put(`http://13.201.98.216:4000/api/adopting/adoption-requests/${id}`, {
        status: 'Adopted',
      });
