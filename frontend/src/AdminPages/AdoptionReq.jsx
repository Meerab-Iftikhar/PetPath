...
const AdoptionReq = () => {
  ...
  useEffect(() => {
    const fetchAdoptionRequests = async () => {
      try {
        const response = await axios.get('http://localhost:4000/adoption-requests');
  ...
  const handleReject = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/adoption-requests/${id}`);
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const response = await axios.put(`http://localhost:4000/adoption-requests/${id}`, {
        status: 'Adopted',
      });
