...
const AllReq = () => {
    ...
    useEffect(() => {
        const fetchPendingRequests = async () => {
            try {
                const response = await axios.get('http://3.111.51.68:5000/api/adoption/get-adoption-requests');
    ...
    const handleApprove = async (id) => {
        try {
            await axios.put(`http://3.111.51.68:5000/api/adoption/approve-adoption/${id}`);
    ...
    const handleReject = async (id) => {
        try {
            ...
            await axios.delete(`http://3.111.51.68:5000/api/adoption/reject-adoption/${id}`);
            setPendingRequests(pendingRequests.filter(request => request._id !== id));
        } catch (error) {
